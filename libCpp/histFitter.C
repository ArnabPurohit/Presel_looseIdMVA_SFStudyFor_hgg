...
void tnpFitter::fits(bool mcTruth,string title) {
  ...
  pPass->SetName("M_{ee} in GeV");
  pFail->SetName("M_{ee} in GeV");
  ...
  TCanvas c_pass("c_pass","c_pass",1100,768);
  // c.Divide(2,1);//3,1
  ...
  pPass->Draw();
  //c.Update();
  TCanvas c_fail("c_fail","c_fail",1100,768);
  //c.cd(2);
  pFail->Draw();
  //c.Update();
  _fOut->cd();
  //  pPass->Write();
  // pFail->Write();
  c_pass.Write(TString::Format("%s_Canv_pass",_histname_base.c_str()),TObject::kOverwrite);
  c_fail.Write(TString::Format("%s_Canv_fail",_histname_base.c_str()),TObject::kOverwrite);
