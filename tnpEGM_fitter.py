...
if args.createHists:
    ...
    for sampleType in tnpConf.samplesDef.keys():
        ...
        if sampleType == args.sample or args.sample == 'all' :
            ...
            sample.dump()
            #var = { 'name' : 'pair_mass', 'nbins' : 80, 'min' : 50, 'max': 130 } #AP
            var = { 'name' : 'pair_mass', 'nbins' : 100, 'min' : 60, 'max': 120 }
            if sample.mcTruth:
                var = { 'name' : 'pair_mass', 'nbins' : 100, 'min' : 60, 'max': 120 }
...
if args.sumUp:
   ...
   #     info['tagSel'   ] = tnpConf.samplesDef['tagSel'].histFile
    ...
    for ib in range(len(tnpBins['bins'])):
        ...
        print tnpBins['bins'][ib]['title'].split(';')[1].split('<')
        if isPresel:
            v1Range = tnpBins['bins'][ib]['title'].split(';')[0].split('<')
            v2Range = tnpBins['bins'][ib]['title'].split(';')[1].split('<')
        else:
        ...
        astr =  '%+8.3f\t%+8.3f\t%+8.3f\t%+8.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f' % (
